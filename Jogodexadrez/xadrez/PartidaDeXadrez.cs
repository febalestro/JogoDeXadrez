...
namespace xadrez
{
    internal class PartidaDeXadrez
    {
        ...
        public Peca ExecutaMovimento(Posicao origem, Posicao destino)
        {
            ...
            p.IncrementarQteMovimentos();//quando tira a peça é preciso incrementar a quantidade de movimentos
            Peca pecaCapturada = Tab.RetirarPeca(destino); //se movo a peça para uma posição que tem outra, preciso capturar essa peça;
            Tab.ColocarPeca(p, destino); //colocar a peça no destino
            ...
            if (p is Peao)
            {
                if (origem.Coluna != destino.Coluna && pecaCapturada == null)//significa que moveu na diagonal
                {
                    Posicao posP;
                    if (p.Cor == Cor.Branca)
                    {
        ...
        public HashSet<Peca> PecasEmJogo(Cor cor)
        {
            ...
            aux.ExceptWith(PecasCapturadas(cor)); //retiro as peças capturadas dessa cor
            return aux;
        }
        ...
        {
            if (!EstaEmXeque(cor)) //teste: se não está em xeque também não está em xeque mate
            {
                return false;
            }
            foreach (Peca x in PecasEmJogo(cor)) //varrer todo o conjunto de peças x dessa cor que, se movidas, tiram do xeque
            {
                bool[,] mat = x.MovimentosPossiveis(); //cria a matriz com todos os movimentos possiveis dessa peça x
                for (int i = 0; i < Tab.Linhas; i++)
                {
                    for (int j = 0; j < Tab.Colunas; j++)
                    {
                        if (mat[i, j])
                        {
                            ...
                            Peca pecaCapturada = ExecutaMovimento(origem, destino);//tenta executar movimento da peça x da pos que ela está para a pos destino
                            bool testeXeque = EstaEmXeque(cor); //testa se a peça coloca o rei em xeque
                            DesfazMovimento(origem, destino, pecaCapturada);
                            if (!testeXeque)
                            {
                                return false;
